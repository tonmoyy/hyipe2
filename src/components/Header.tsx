...
export default function Header() {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between h-12 px-6 bg-[#0D0D0B] text-white text-[11px] tracking-[0.08em] uppercase font-medium">
            {/* Brand */}
            <Link href="/" className="font-['Playfair_Display'] text-[18px] tracking-[0.02em] normal-case font-bold text-white hover:text-gray-300 transition-colors">
                HYIPE
            </Link>

            {/* Navigation links (like .pages in wireframe) */}
            <nav className="flex items-center gap-4">
                ...
                {user ? (
                    <>
            <span className="text-gray-400 normal-case text-[11px] tracking-normal">
              {profile?.full_name || user.email}
            </span>
                        <Link href={`/dashboard/${profile?.role}/profile`} className="text-white/60 hover:text-white transition-colors">
                            Dashboard
                        </Link>
                      ...
                      {unreadCount > 9 ? '9+' : unreadCount}
                                                    ...
                                                    messages.map((m) => (
                                                        <Link
                                                            ...
                                                            className={`block px-4 py-3 border-b border-gray-50 transition-colors hover:bg-gray-100 ${
                                                                !m.read ? 'bg-blue-100 border-l-4 border-l-blue-600 font-semibold' : ''
                                                            }`}
                                                        >
                                                            <div className="flex justify-between items-start">
                                                                <p className="text-sm line-clamp-2">{m.content}</p>
                                                                {!m.read && <span className="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0 ml-1 mt-0.5" />}
                                                            </div>
                        ...
                        <button
                            ...
                            className="text-red-400 hover:text-red-300 hover:underline p-0 h-auto uppercase text-[11px] tracking-[0.08em]"
                        >
                            Sign Out
                        </button>
                    </>
